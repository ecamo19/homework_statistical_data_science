...
__The confusion matrix shows that the logistic regression model in the train dataset classified correctly as No 427 out of 524 observations while it classified correctly as Yes 237 out of 310 observations__ 
...
__mtry for the random forest is defined as $\sqrt{predictors}$ __
...
```{r}
# Get test predictions for random forest
...
```{r}
# Test accuracy rate for random forest
...
probs <- predict(random_forest_titanic, newdata = titanic[-index,], type = "prob")
pred <- predict(random_forest_titanic, newdata = titanic[-index,])

head(as.data.frame(cbind(probs,pred)))
```


```{r}
testing_data$random_forest_prob_no <- probs[,1]
testing_data$random_forest_prob_yes <- probs[,2]
...
The sensitivity of the model is the rate that the event of interest is predicted correctly for all samples having the event. (True positive rate)
It measures the accuracy in the event population
```{r}
...
```{r}
1 - (specificity(data = testing_data$random_forest_pred,
            reference = testing_data$survived,
            negative = "No"))
```
...
```{r}
roc_curve <- roc(response  = testing_data$survived,
                 predictor = testing_data$random_forest_prob_no,
                 levels    = rev(levels(testing_data$survived)))
```

```{r}
plot(roc_curve,legacy.axes = TRUE, xlim = c(1,0))
```
